...
function Chart({ data1, data2, width, height }) {
  ...
  useLayoutEffect(() => {
    // https://www.amcharts.com/docs/v5/getting-started/#Root_element
    let root = am5.Root.new("chartdivv1l");
    ...
    const avgbp = [
      {
        ...
        mvalue:
          Math.round(parseFloat(data2.l_bpSaved / data2.l_bpFaced) * 1000) / 10,
      },
      {
        ...
        mvalue:
          Math.round(
            parseFloat((data2.w_bpFaced - data2.w_bpSaved) / data2.w_bpFaced) *
              1000
          ) / 10,
      },
    ];
    ...
    const avgsv = [
      {
        ...
        mvalue:
          Math.round(
            parseFloat(data2.l_2ndWon / (data2.l_svpt - data2.l_1stIn)) * 1000
          ) / 10,
      },
      {
        ...
        mvalue:
          Math.round(parseFloat(data2.l_1stWon / data2.l_1stIn) * 1000) / 10,
      },
      {
        category: "1st Serve In %",
        avvalue: Math.round(parseFloat(stat.avg_fstin) * 1000) / 10,
        mvalue:
          Math.round(parseFloat(data2.l_1stIn / data2.l_svpt) * 1000) / 10,
      },
    ];
